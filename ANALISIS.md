...
- a credential-safe T3N adapter slice that reads organisation policy and agent metadata;
- a local staged draft bound to a hash of the observed T3N snapshot, with no provider write path;
- no external requests and no secrets.

## What is not claimed

The demo is not connected to a real filesystem, CRM, bank, or subscription provider, and no external business action has
occurred. A T3N sandbox agent identity has been provisioned and its card published for the challenge, but that identity
does not turn the synthetic demo into a real provider integration. The production version must implement provider-specific
snapshots, idempotency, authorization, and error recovery before it can safely mutate real data.
...
The next slice after the current read-only adapter should add a human approval object and provider-specific snapshot
freshness checks around one reversible staged operation. It must still avoid direct destructive mutations. The provider
should expose an explicit reverse operation before any commit path is enabled.
